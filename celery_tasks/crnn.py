...
class CRNN_Model:
    ...
    def preprocess_input_dev(self,  img_list, img_folder:str, n_img:int):
        img_set = []
    ...
    def prepare_prediction_dev(self, pred, n_img):
        # Decode the predictions
        decoded = ctc_decode(pred, input_length=np.ones(pred.shape[0]) * pred.shape[1], greedy=True)[0][0]
        # Convert the decoded predictions to numpy array
        decoded = get_value(decoded)
    ...
    def words_predict_dev(self,img_list, img_folder:str, n_imgs:int):
        try:
           
            input_set = self.preprocess_input_dev(img_list, img_folder, n_imgs)

            pred = self.model.predict(np.array(input_set).reshape(-1, 256, 64, 1))
            
            result = self.prepare_prediction_dev(pred,n_imgs)
